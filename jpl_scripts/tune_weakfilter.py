...
def count_common_lines(lines1, lines2, tolerance=1e-8):
    ...
    """
    # Verify input shapes
    assert lines1.shape[1:] == (2, 2) and lines2.shape[1:] == (2, 2), \
...
class WeakFilterAutoTuner:
    ...
    def __init__(self, conf, dataset, jpl_model, dlsd_model, exp_name, device='cpu') -> None:
        ...
        self.jpl_model_used = (not (self.conf["use_deeplsd_df"] and self.conf["use_deeplsd_af"] and self.conf["use_deeplsd_keypoints"]))
        ...
        self.current_config = self.generate_random_config() if self.do_rand_init else { k:v[1] for k,v in self.conf["parameters"].items()}
    ...
    def calculate_metric_single_image(self, line_extractor_out_lines: torch.Tensor, gt_dlsd_lines: torch.Tensor) -> float:
        ...
        count_lines_part = - (self.conf["metric"]["weight_num_lines"] * more_lines_factor * abs(num_lines - num_dlsd_lines))
    ...
    def save_state(self):
        ...
        """
        out_path = Path(__file__).parent / "autotune_out" / self.exp_name / f"results_{self.num_iter}.npy"
    ...
    def step(self):
        ...
        if best_neigh_metric > self.best_metric_value and not best_neigh_metric < self.minimum_accepted_metric_value:
            ...
            self.current_config = self.generate_random_config() # TODO: add check that was not visited yet
    ...
    def apply_conf_change_from_neighbour(self, conf: dict, neighbour: tuple):
        ...
        """
        n_conf = conf.copy()
        n_conf[neighbour[0]] = float(np.round(neighbour[1], 2)) if neighbour[0] != 'samples' else int(neighbour[1])
        return n_conf
    ...
    def calculate_metric_for_config(self, config):
        ...
        for i, idx in enumerate(self.eval_indices):
            ...
            res = self.calculate_metric_single_image(out["lines"], data["dlsd_lines"].clone())
            results[i] = res
            if res < self.minimum_accepted_metric_value:
                # each image must have a minimum metric value 
                break
        overall_metric = self.calculate_overall_metric(results, mode=self.conf['metric']['aggregation'])
