...
class POLD2_MLP_Dataset(BaseDataset):
    ...
    def _init(self, conf):
        ...
        if not (data_dir / "positives.npy").exists() or not (data_dir / "negatives.npy").exists():
        ...
        labels = torch.cat((torch.ones(positives.shape[0]), torch.zeros(negatives.shape[0])), axis=0)


        if conf.shuffle_seed is not None:
            idxs = torch.randperm(samples.shape[0], generator=torch.Generator().manual_seed(conf.shuffle_seed))
        ...
        logger.info(f"Loaded POLD2-MLP dataset with total {len(samples)} samples, Train: {len(train_samples)}, Val: {len(val_samples)}")
    ...
    def generate_data(self, conf: OmegaConf, data_dir: Path):
        ...
        if not gen_conf.use_df and not gen_conf.use_af:
            raise ValueError("At least one of the fields (distance or angle) must be used.")
        ...
        for file_path in tqdm(fps, desc="Generating data"):
            distance_map, angle_map, lines, img_shape = get_line_from_image(file_path, net)
            ...
            for _ in range(conf.generate.num_negative_per_image):
                p1 = np.array(
                    [np.random.randint(0, img_shape[1]), np.random.randint(0, img_shape[0])]
                )
                p2 = np.array(
                    [np.random.randint(0, img_shape[1]), np.random.randint(0, img_shape[0])]
                )
                negatives.append(
                    datasetEntryFromPoints(p1, p2, distance_map, angle_map, blend, img_shape)
                )
...
if __name__ == "__main__":
    ...
    conf = OmegaConf.load(args.conf) if args.conf is not None else POLD2_MLP_Dataset.default_conf
