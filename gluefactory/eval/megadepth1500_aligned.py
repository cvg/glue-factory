...
from geocalib.models.extractor import GeoCalib
...
class MegaDepth1500Pipeline(EvalPipeline):
    default_conf = {
        "data": {
            ...
            "extra_data": "{scene}/rpfov.txt",
            "extra_keys": ["roll", "pitch", "vfov"],
            "depth_format": "h5",
    ...
    def run_eval(self, loader, pred_file):
        ...
        for i, data in enumerate(tqdm(loader)):
            ...
            if self.conf.eval.use_geocalib:
                pred["camera0"] = Camera(
                    self.calib.extract(data["view0"]["image"][0])["camera"][:, :6].cpu()
                )
                pred["camera1"] = Camera(
                    self.calib.extract(data["view1"]["image"][0])["camera"][:, :6].cpu()
                )
                cameras_est = torch.stack([pred["camera0"], pred["camera1"]])
                cameras_gt = torch.stack(
                    [data["view0"]["camera"], data["view0"]["camera"]]
                )
                focal_errors = cameras_est.f - cameras_gt.f
                results_i["focal_error"] = focal_errors.abs().max().item()
                results_i["focal_error_rel"] = (
                    focal_errors.abs() / cameras_gt.f
                ).max().item() * 100.0
