...
def keep_shared_points(kp, desc, H, img_shape, keep_k_points=1000):
    ...
    selected_kp, selected_desc = select_k_best(
        selected_kp, selected_desc, keep_k_points
    )
...
def compute_matching_score(
    ...
    correct_warped = np.all(
        (warped_m_kp1 >= 0) & (warped_m_kp1 <= (np.array(img_shape) - 1)), axis=-1
    )
    ...
    for thr in thresh:
        matching_score.append(
            np.sum((dist < thr) * correct_warped) / np.maximum(correct_warped.sum(), 1)
        )

    return matching_score


def compute_homography(kp0, kp1, desc0, desc1, H_gt, img_shape, keep_k_points=1000):
    ...
    kp0_corr, desc0_corr = keep_shared_points(
        kp0, desc0, H_gt, img_shape, keep_k_points
    )
    kp1_corr, desc1_corr = keep_shared_points(
        kp1, desc1, np.linalg.inv(H_gt), img_shape, keep_k_points
    )
