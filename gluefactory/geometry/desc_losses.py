...
def get_sparse_desc(kp0, valid_kp0, H, desc0, desc1):
    ...
    valid_desc0 = (
        F.grid_sample(desc0, grid0, align_corners=False)
        .permute(0, 2, 3, 1)
        .reshape(b_size, n_points, -1)
    )
    valid_desc0 = F.normalize(valid_desc0, dim=2)
    valid_desc1 = (
        F.grid_sample(desc1, grid1, align_corners=False)
        .permute(0, 2, 3, 1)
        .reshape(b_size, n_points, -1)
    )
...
def triplet_loss(
    kp0, valid_kp0, H, dense_desc0, dense_desc1, margin=1.0, dist_threshold=8
):
...
def nll_loss(kp0, valid_kp0, H, dense_desc0, dense_desc1, temperature=1.0):
    ...
    # Dual softmax
    prob_desc = (
        F.log_softmax(prob_desc / temperature, dim=2)
        + F.log_softmax(prob_desc / temperature, dim=1)
    ) / 2
...
def caps_loss(kp, kp_scores, descriptors, H, dense_desc_w, s=8):
    ...
    assert desc_grid.shape[2] == prod(
        grid.shape[:2]
    ), f"{desc_grid.shape[2]} and {prod(grid.shape[:2])} should be equal"
    ...
    inv_std = inv_std / inv_std.sum(
        dim=1, keepdim=True
    )  # it was a mean here in the original CAPS
    ...
    loss = (loss * inv_std * valid_mask).sum(dim=1) / (valid_mask.sum(dim=1) + eps)
    return loss


def caps_window_loss(
    kp,
    kp_scores,
    descriptors,
    H,
    ...
    desc_grid = F.grid_sample(dense_desc_w, keypoints_to_grid(grid, (h, w)))
    ...
    if std:
        var = (grid**2 * corr[..., None]).sum(dim=2) - expect_coord**2
        inv_std = 1.0 / torch.clamp(
            torch.sqrt(torch.clamp(var, min=eps)).sum(dim=-1), min=eps
        )  # [B, n_kp]
        inv_std = inv_std / inv_std.sum(
            dim=1, keepdim=True
        )  # it was a mean here in the original CAPS
