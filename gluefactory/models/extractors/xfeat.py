...
class XFeatModel(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> dict:
        ...
        kpt_logit_map = self.keypoint_head(self._unfold2d(x, ws=8)) #Keypoint map logits
...
class XFeat(BaseModel):
    ...
    checkpoint_url = "https://github.com/verlab/accelerated_features/raw/main/weights/xfeat.pt"
    ...
    def _init(self, conf: OmegaConf):
        ...
        if self.conf.preprocess:
            assert self.conf.get_sparse_outputs, "Preprocessing requires get_sparse_outputs=True"
        ...
        if self.conf.pretrained:
            ...
            if self.conf.model_name != ModelName.XFEAT_DENSE.value:
                state_dict = {k: v for k, v in state_dict.items() if 'fine_matcher' not in k}
    ...
    def get_sparse_outputs(self, data: dict, net_output: dict, rh: float=1.0, rw: float=1.0) -> dict:
        """
        ...
        scores = (_nearest(kpt_score_map, mkpts, H, W) * _bilinear(rel_map, mkpts, H, W)).squeeze(-1)
        ...
        if self.conf.preprocess:
            mkpts = mkpts * torch.tensor(
                    [rw,rh], device=mkpts.device).view(1, 1, -1)
