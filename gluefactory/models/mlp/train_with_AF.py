...
class MLPDataset(Dataset):
    def __init__(self, isTrain=True):
        if isTrain:
            self.positives = torch.from_numpy(np.load(DATA_PATH / "mlp_data/positives.npy")).float()
            self.negatives = torch.from_numpy(np.load(DATA_PATH / "mlp_data/negatives.npy")).float()
        else:
            self.positives = torch.from_numpy(
                np.load(DATA_PATH / "mlp_data/positives_test.npy")
            ).float()
            self.negatives = torch.from_numpy(
                np.load(DATA_PATH / "mlp_data/negatives_test.npy")
            ).float()
        print(f"Positives: {self.positives.shape} Negatives: {self.negatives.shape}")
...
def train(model, train_data, val_data, num_epochs=5):
    optimizer = Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=1e-4)

    for _ in torch.arange(1, num_epochs + 1):
        train_loss = 0.0
    ...
    torch.save(model.state_dict(), DATA_PATH / "mlp_data/mlp.pth")
...
if __name__ == "__main__":
    ...
    final_model = train(model, train_dataloader, validation_dataloder, num_epochs=20)
    ...
    cm_display.plot()
    plt.savefig(DATA_PATH / "mlp_data/confusion_matrix.png")
