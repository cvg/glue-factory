...
class CrossBlock(nn.Module):
    def __init__(
        ...
        self.dim_head = embed_dim // num_heads // 2
        self.scale = self.dim_head**-0.5
        inner_dim = self.dim_head * num_heads * 2
        ...
        self.inner_attn = MultiheadDiffAttn(embed_dim=inner_dim, depth=0, num_heads=num_heads, num_kv_heads=None)
    ...
    def forward(
        ...
        if self.flash is not None and qk0.device.type == "cuda":
            ...
            qk0 = qk0.view(bsz, tgt_len, 2 * self.heads, self.dim_head).transpose(1, 2)         # [B, 2*H, N, D]
            qk1 = qk1.view(bsz, tgt_len, 2 * self.heads, self.dim_head).transpose(1, 2)         # [B, 2*H, N, D]
            src_len = tgt_len
            v0 = v0.view(bsz, src_len, self.heads, 2 * self.dim_head).transpose(1, 2)      # [B, H, N, 2*D]
            v1 = v1.view(bsz, src_len, self.heads, 2 * self.dim_head).transpose(1, 2)      # [B, H, N, 2*D]


            m0 = self.inner_attn(qk0, qk1, v1, attn_mask=mask)  # [B, N, D]
            m1 = self.inner_attn(qk1, qk0, v0, attn_mask=mask)  # [B, N, D]
